...
interface CropperProps {
  handleImg: (newVal: string) => void;
  img: string;
}

function CropperV2(props: CropperProps) {
  const { img, handleImg } = props;
  ...
  const handleZoomChange = (newVal: any): void => {
    setZoom(newVal);
  };
  ...
  const handleCrop = (): void => {
    if (typeof crop !== 'undefined') {
      const result = crop.getCroppedCanvas().toDataURL();
      handleImg(result);
      setOpen(!open);
      setZoom(0);
    }
          ...
          <div className="relative w-full h-full">
            ...
            <div className="text-center w-[616px] h-[675px] bg-white rounded-1-0 px-[126px] py-3-0 absolute top-50-percent left-50-percent -translate-x-50-percent -translate-y-50-percent">
              ...
              <div className="mt-2-0 h-2-0 flex items-center">
                ...
                <InputSlider onChange={handleZoomChange} value={zoom} />
